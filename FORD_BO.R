...
FORD_BO = function(mu_pos,mu_neg,mu,sigma,N,prev,t,c1n,c1p,eff_bound){
  ...
  posbare = mean(pose)
  sigmapose = sd(pose)
  test.stat.pos.ia = (posbar1-mu)/(sqrt(2/N1p))
  test.stat.neg.ia = (negbar1-mu)/(sqrt(2/N1n))
  test.stat.pool.ia = (pool1-mu)/(sqrt(2/(N1n+N1p)))
  ...
  # PETE is the probability of interim efficacy stopping
  # SS is the total sample size of the study, rejpos and rejO are decisions to reject the positive group or overall population.
  return(c(PET,PET.n,PETE,SS,rejpos,rejO))
}
